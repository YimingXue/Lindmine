...
class C3F4_CNN_RON(nn.Module):
    def __init__(self, config):
        super(C3F4_CNN_RON, self).__init__()
        self.config = config
    ...
    def forward(self, x):
        ...
        c2_fpn = self.conv2_fpn(c1) # N x 350 x H/2 x w/2
        c2 = c2_fpn + c2 # N x 350 x H/2 x w/2
        
        c3 = self.conv3(c2) # N x 350 x H/4 x w/4
        c3_fpn = self.conv3_fpn(c2) # N x 350 x H/4 x w/4
        c3 = c3_fpn + c3 # N x 350 x H/4 x w/4
